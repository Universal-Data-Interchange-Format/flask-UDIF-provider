...
def create_udif_file():
    ...
    if json_data is None:
        return jsonify({'Error': 'user_id, requested_id, username, requested_datetime are required.'})
    if 'user_id' not in json_data:
        return jsonify({'Error': 'user_id is required'})
    if 'username' not in json_data:
        return jsonify({'Error': 'username is required'})
    if 'requested_datetime' not in json_data:
        return jsonify({'Error': 'requested_datetime is required'})
    if 'requested_id' not in json_data:
        return jsonify({'Error': 'requested_id is required'})

    user_id = json_data['user_id']
    requested_id = json_data['requested_id']
    username = json_data['username']
    requested_datetime = json_data['requested_datetime']
    try:
        celery_process_udif.delay(user_id, requested_id, username, requested_datetime)
